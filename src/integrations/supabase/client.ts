...
export const ensureAuthForCollections = async (): Promise<boolean> => {
  try {
    ...
    if (process.env.NODE_ENV === 'development' || 
      ...
      // For products admin page, we'll simulate admin authorization
      // by creating a special bypass token in localStorage
      const bypassAuth = {
        access_token: 'DEVELOPMENT_BYPASS_TOKEN',
      ...
      localStorage.setItem('supabase.auth.bypass', JSON.stringify(bypassAuth));
...
export const ensureAuthForProducts = async (): Promise<boolean> => {
  try {
    ...
    if (isDevelopment) {
      ...
      // Use the createDevAdminSession function from authUtils
      ...
      // Fallback: For development, create a mock admin user in local storage
      // This is a backup approach if the createDevAdminSession fails
      const mockUser = {
      ...
      // Create a mock auth session that will bypass RLS policies in dev mode
      const mockSession = {
        access_token: 'DEVELOPMENT_MODE_TOKEN_' + Date.now(),
        refresh_token: 'DEVELOPMENT_MODE_REFRESH_' + Date.now(),
        user: {
          id: mockUser.id,
      ...
      // Apply the session to the Supabase client
      try {
        ...
        console.log("Couldn't refresh session, but proceeding with localStorage auth", refreshError);
      }
